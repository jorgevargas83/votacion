...
(function(){
  ...
  createBtn.onclick = async function(){
    try{
      ...
      if (rawJudges){
        ...
        if (codes.length){
          var rows = codes.map(function(code){ return { poll_id: poll.id, user_id: code }; });
          var j = await supabase.from("poll_judges").insert(rows);
          if (j.error) console.warn("No se pudieron insertar algunos jueces:", j.error);
        }
      }
      ...
      resultsBtn.onclick = function(){ location.href = "results.html?poll=" + poll.id; };
  ...
  async function loadPolls(){
    var tbody = document.getElementById("pollRows");
