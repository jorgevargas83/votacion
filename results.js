// results.js (sin imports). Requiere window.App
(function(){
  const { supabase, qs, $, fmt, WEIGHT_JUDGES, WEIGHT_PUBLIC, toast } = window.App;
  ...
  async function pickOrLoad(){
    ...
    $("#btnOpen").onclick = async ()=>{
      const { error } = await supabase.from("polls").update({ is_open: !(poll.is_open===true) }).eq("id", pollId);
  ...
  function render(votes){
    ...
    $("#listJ").innerHTML = judges.map((v,i)=><li>Juez #${i+1}: ${fmt(v.score)}</li>).join("") || "<li class='small'>—</li>";
    $("#listP").innerHTML = publics.map((v,i)=><li>Público #${i+1}: ${fmt(v.score)}</li>).join("") || "<li class='small'>—</li>";
  }

  async function loadVotes(){
    const { data, error } = await supabase.from("votes").select("role, score").eq("poll_id", pollId);
