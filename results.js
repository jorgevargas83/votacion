...
function render(votes){
  ...
  // Listas simples (sin code, por privacidad/joins)
  $("#listJ").innerHTML = judges.map((_,i)=><li>Juez #${i+1}: ${fmt(judges[i].score)}</li>).join("") || "<li class='small'>—</li>";
  $("#listP").innerHTML = publics.map((_,i)=><li>Público #${i+1}: ${fmt(publics[i].score)}</li>).join("") || "<li class='small'>—</li>";
}
